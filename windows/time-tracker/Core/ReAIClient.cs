...
public sealed class ReAIClient : IDisposable
{
    ...
    public async Task<JsonElement> Request(string path, string token, int? company = null, string? payload = null, CancellationToken cancellation = default)
    {
        ...
        if (!response.IsSuccessStatusCode)
        {
            string message = response.StatusCode switch
            {
                ...
                HttpStatusCode.Forbidden => "Your account needs timesheet permission in this company.",
                HttpStatusCode.Conflict => "A timer is already running, time tracking is disabled, or your account has no linked employee. Refresh to check.",
    ...
    public async Task<T> Get<T>(string path, string token, int company) =>
        (await Request(path, token, company)).Deserialize<T>(Json.Options) ?? throw new InvalidDataException("Invalid ReAI response.");
    public async Task<T> Post<T>(string path, object body, string token, int company) =>
        (await Request(path, token, company, JsonSerializer.Serialize(body, Json.Options))).Deserialize<T>(Json.Options) ?? throw new InvalidDataException("Invalid ReAI response.");
    public async Task<string> Connect(Action<string, Uri> showCode, CancellationToken cancellation)
    {
        ...
        const string client = "time-tracker-windows";
        using var response = await Post("authorize", new() { ["client_id"] = client, ["client_name"] = "ReAI Time Tracker for Windows" });
