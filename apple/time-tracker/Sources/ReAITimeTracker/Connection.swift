...
actor Connection {
    func authorize(showCode: @escaping @Sendable (String) async -> Void) async throws -> String {
        ...
        let (data, response) = try await session.data(for: form("authorize", ["client_id": "time-tracker", "client_name": "ReAI Time Tracker"]))
        ...
        while ContinuousClock.now < deadline {
            ...
            do {
                pair = try await session.data(for: form("token", [
                    "client_id": "time-tracker", "device_code": grant.deviceCode,
